...
var BarcodeScanner = {
	...
	MWBinitDecoder : function() {
		...
		BarcodeScanner.WMBcreateFrontView();
		BarcodeScanner.WMBaddFrontView();
	...
	MWBenableFlash: function(){
		scanner.MWBenableFlash();
	},
	...
	WMBenableFlash: function(){
		scanner.MWBenableFlash();
	},
	WMBcreateFrontView: function(){	
	...
	flashButton.addEventListener('click', function(){
		BarcodeScanner.MWBenableFlash();
	});
	overlay.add(flashButton);	
	//closeButton
	var exitButton=Ti.UI.createButton({
		backgroundImage: 'close_button',
		title: 'exit', textAlign: 'center',
    	color: '#000', backgroundColor: '#f00', style: 0,
   		font: { fontWeight: 'bold', fontSize: 16 },
    	borderColor: '#000', borderRadius: 10, borderWidth: 1,
		top: 40,
		right: 20,
...
//zoomLevelButton
	var zoomButton=Ti.UI.createButton({
		title: 'zoom', textAlign: 'center',
   		color: '#000', backgroundColor: '#fff', style: 0,
    	font: { fontWeight: 'bold', fontSize: 16 },
    	borderColor: '#000', borderRadius: 10, borderWidth: 1,
		backgroundImage: 'zoom.png',
		bottom: 40,
		left: 20,
		width: 80,
		height: 80
	...
	zoomButton.addEventListener('click', function(){
		BarcodeScanner.MWBsetLevel();
	});
	overlay.add(zoomButton);
	},
	
	WMBaddFrontView: function(){
		scanner.addOverlayView({
			overlay: overlay
		});		
	}
};
...
var startScanning = function() {
	...
	var closeCameraOnDetection = false;
